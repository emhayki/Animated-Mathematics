...
class MalteseCross(Scene):
    def construct(self):
        self.camera.background_color = BLACK

        # Title and equation
        title = Text("The Maltese Cross", color=WHITE, weight=BOLD).scale(1.2).move_to(UP * 3.5)
        equation = MathTex(
        ...
        # Parameter values and corresponding colors
        a_values = [1, 1.5, 2, 3, 4, 5, 7, 10, 12]
        colors = color_gradient([BLUE, TEAL, GREEN, YELLOW, ORANGE, RED], len(a_values))

        # Parameter label
        a_label = MathTex("a = 0.00", color=YELLOW).scale(0.85).next_to(equation, DOWN, buff=0.4)
        self.play(Write(a_label))

        # Animate the family of curves
