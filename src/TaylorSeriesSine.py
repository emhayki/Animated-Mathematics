...
class TaylorSeriesSine(Scene):
    def construct(self):
        ...
        equation = MathTex(r"f(x) = \sin(x)", color=YELLOW).scale(1.0).next_to(title, DOWN, buff=0.4)
        ...
        def taylor_sin(x, n):
            return sum(((-1)**(k//2)) * x**k / math.factorial(k) for k in range(n+1) if k % 2 == 1)

        for i, deg in enumerate([1, 3, 5, 7]):
            approx = axes.plot(lambda x: taylor_sin(x, deg), color=BLUE_D)
