...
class PolarVsCartesian1(Scene):
    def construct(self):
        ...
        cartesian_label = MathTex(
            r"y(x) = 1 + 1.8 \sin(\sin(1.8x))",
            font_size=36
        ).move_to(UP * 3.8)

        polar_label = MathTex(
            r"r(\theta) = 1 + 1.8 \sin(\sin(1.8\theta))",
            font_size=36
        ).move_to(DOWN*-0.1)
        ...
        cartesian_func = cartesian_axes.plot(
            lambda x: 1 + 1.8 * np.sin(np.sin(1.8 * x)),
            x_range=[0, 10 * PI],
            color=BLUE
        )

        polar_func = polar_axes.plot_polar_graph(
            lambda theta: 1 + 1.8 * np.sin(np.sin(1.8 * theta)),
            theta_range=[0, 10 * PI],
            color=YELLOW,
            stroke_width=2
        )
